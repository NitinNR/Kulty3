...
export const AdminEventsPage = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [eventsRes, venuesRes] = await Promise.all([getAdminEvents(), getAdminVenues()]);
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <tbody className="divide-y divide-gray-50">
                {events.map((event) => (
                  <tr key={event._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-700">{event.venueId?.name || '—'}</td>
            ...
            {events.length === 0 && (
              <div className="text-center py-12 text-gray-400">No events yet. Add one!</div>
            )}
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-screen overflow-y-auto">
            ...
            <div className="flex gap-3 p-6 pt-0">
              <button onClick={() => setModal(null)} className="flex-1 py-2.5 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={saving || !form.title.trim() || !form.venueId}
